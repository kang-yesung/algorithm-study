...
public class 벽부수고이동하기 {
    public static void main(String[] args) throws IOException {
        ...
        int count = bfs(N, M, map);

        System.out.println(count);
    }

    static int bfs(int N, int M, String[] map){
        int[] dr = {0,1,0,-1};
        int[] dc = {1,0,-1,0};
        ...
        visited[0][0][0] = true;
        queue.offer(new int[]{0,0,1,0}); //세로, 가로, 거리, 벽부셨는지 0 = X, 1 = O
        ...
        while (!queue.isEmpty()){
            ...
            for(int i = 0; i < 4; i++){
                ...
                else if (isValid(nr, nc, N, M) && map[nr].charAt(nc) == '1' && wall == 0) {
                    if(!visited[nr][nc][1]){
